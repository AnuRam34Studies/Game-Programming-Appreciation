...
public class Player {            
  ...
  private static final int DY = 8;   //32// amount of Y pixels to move in one keystroke
  ...
  public Player (JPanel panel, TileMap t, BackgroundManager b) {
      ...
      playerRightImage = ImageManager.loadImage("images/Wushu.gif");
...
*/
  ...
  public synchronized void move (int direction) {
      ...
      if (direction == 1) 
      { // move up
        playerImage = playerRightImage; // Assign the image representing the player facing up
        newY = y - DY; // Calculate the new Y position after moving up
        tilePos = collidesWithTileUp (x, newY+5);    
            if (newY < 0) { // Check if the new Y position is beyond the top edge of the map
                //int tileMapHeight = tileMap.getHeightPixels(); // Get the height of the tile map
                y = 10; //tileMapHeight - playerHeight; // Adjust the Y position to keep the player within the map boundaries
                return; // Exit the method because the player has reached the top edge of the map
            }
          ...
          {        
              ...
              playerImage = playerRightImage;
              int playerHeight = playerImage.getHeight(null); // Get the height of the player image
              newY = y + DY;
              int tileMapHeight = tileMap.getHeightPixels();
              if (newY >450) {
                y = 450;
                return;
              }
        
              tilePos = collidesWithTile(x, newY+playerHeight);
      }
      ...
      if (tilePos != null) {  
        if (direction == 1) {
            ...
            y = y;// ((int) tilePos.getY() ) -TILE_SIZE;       // keep flush with upside of tile
        }
        else if (direction == 2) {
            ...
            System.out.println (": Collision going down");
              int playerHeight= playerImage.getHeight(null);
              y =y; //((int) tilePos.getY())* TILE_SIZE + playerHeight; // keep flush with downside of tile
        }
      }
      else {
          /*if (direction == 1) {
