...
public class Attacker2{
    ...
    // Define the desired distance between the attacker and the player along the x-axis
    /*int desiredDistanceX = 100; // Adjust this value as needed

    // Calculate the distance along the x-axis between the attacker and the player
    int distanceX = attackerX - playerX;

    // If the distance is greater than the desired distance, move towards the player
    if (distanceX > desiredDistanceX) {
        // Move left towards the player
        x -= dx;
        // Deactivate projectile
        deActivateProjectile();
    } else if (distanceX < -desiredDistanceX) {
